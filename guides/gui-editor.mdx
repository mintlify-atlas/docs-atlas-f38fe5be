...
| `sv_ttk` | Applies the Sun Valley (Windows 11-style) theme to all Tkinter widgets |
...
The GUI editor is installed optionally alongside the main library. When you run `py2html_installer.py`, the installer window presents a **"Install Py2Html GUI"** checkbox. Checking it before clicking **Install** copies `py2htmlGUI.py` into the same directory as `py2html/__init__.py`.
...
<Steps>
  ...
  <Step title="Click Install">
    Click the **Install** button. The installer downloads `beautifulsoup4` via pip and writes both `py2html/__init__.py` and `py2html/py2htmlGUI.py` to your home directory.
  </Step>
