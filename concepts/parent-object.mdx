---
title: "The Parent Object: py2html's Core Document Model"
sidebarTitle: "Parent Object"
description: "The Parent class is py2html's central object. Every HTML page is a Parent instance that holds an ordered list of element dictionaries rendered to HTML on demand."
---
...
Calling a `Parent` instance directly is shorthand for `TextNode()`. All positional arguments are joined with the `sep` keyword (default `" "`) and appended as a text node. The `escape` keyword (default `True`) controls HTML-escaping.
...
`str(page)` calls `getHTML(format=True)`, which runs the output through BeautifulSoup's `prettify()` for indented, readable HTML. This is the same output that `save()` writes to a file.
